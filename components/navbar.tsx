    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[.06] bg-[#06080d]/75 backdrop-blur-xl">
      <nav className="mx-auto flex h-18 max-w-6xl items-center justify-between px-5 lg:px-8">
        ...
        <div className="hidden items-center gap-7 md:flex">
          {links.map((link) => <a key={link} href={`#${link.toLowerCase()}`} className="text-base text-white transition hover:text-cyan-200">{link}</a>)}
        </div>
        <a href="https://github.com/thanithpol2545" target="_blank" rel="noreferrer" className="hidden items-center gap-1.5 rounded-full border border-white/10 px-4 py-2 text-base font-semibold text-white transition hover:border-cyan-300/40 hover:text-cyan-200 sm:flex">GitHub <ArrowUpRight size={14}/></a>
      ...
      {open && <div className="border-t border-white/[.06] bg-[#080b12] px-5 py-4 md:hidden">
        {links.map((link) => <a onClick={() => setOpen(false)} key={link} href={`#${link.toLowerCase()}`} className="block py-3 text-base text-white">{link}</a>)}
